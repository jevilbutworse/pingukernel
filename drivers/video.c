...
void printchar(char c,char colors,char toblink,bool comd)
{
...
void printtext(char* text,char colors,char toblink,bool comd)
{
	...
	while((c=*text++)!='\0')
	{
		printchar(c,colors,toblink,comd);
	}
}

void next_line_for_app()
{
        int curpos=get_cursor_pos();
        int y=curpos/80;
        int x=curpos%80;
